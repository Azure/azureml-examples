...
_azure_ml_resource_id = "/" + urlparse(os.environ["MLFLOW_TRACKING_URI"]).path.split("/", 3)[3] # Get the ARM Resource ID of the Azure ML Workspace we are running on
...
def _get_access_token(resource):
    ...
    if client_id is not None and re.match("[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}", client_id, re.IGNORECASE) is not None:
...
def _run(command, check=True):
    ...
    try:
        return subprocess.run(command, shell=True, check=check, universal_newlines=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
class LogAnalyticsTelemetry:
    ...
    def __init__(self, log_analytics_resource_id):
        ...
        if log_analytics_resource_id is None:
            config_path = _local_config_path if os.path.exists(_local_config_path) else _global_config_path if os.path.exists(_global_config_path) else None
        ...
        self.log_analytics_resource_id = _sanitize_log_analytics_resource_id(log_analytics_resource_id)
        
        # Get an AAD access token for ARM
        access_token = _get_access_token("https://management.azure.com")
        headers = {"Authorization": "Bearer " + access_token} # [SuppressMessage("Microsoft.Security", "CS001:SecretInline", Justification="No secret")]

        # Get the Log Analytics Customer ID from ARM
        response = requests.get("https://management.azure.com" + self.log_analytics_resource_id + "?api-version=2021-06-01", headers=headers)
        response.raise_for_status()
        self.log_analytics_customer_id = response.json()["properties"]["customerId"]

        # Get the Log Analytics Shared Key from ARM
        response = requests.post("https://management.azure.com" + self.log_analytics_resource_id + "/sharedKeys?api-version=2020-08-01", headers=headers)
        response.raise_for_status()
        ...
        _logger.debug(f"Log Analytics Workspace ARM Resource ID: {self.log_analytics_resource_id}")
    ...
    def _build_signature(self, date, content_length, method, content_type, resource):
        ...
        authorization = "SharedKey {}:{}".format(self.log_analytics_customer_id, encoded_hash)
    ...
    def send(self, log_type, data):
        ...
        response = requests.post("https://" + self.log_analytics_customer_id + ".ods.opinsights.azure.com" + resource + "?api-version=2016-04-01", data=body, headers=headers)
        response.raise_for_status()
        _logger.info(f"Sent {len(data)} telemetry row(s) to table {log_type} in Log Analytics workspace {self.log_analytics_resource_id}")
...
def _send_health(telemetry, type_, status = None, details = None):
    telemetry.send(
        ...
        [{
            ...
            "Status": status if status is not None else "", # Enum: Started, Succeeded, Failed, ''
...
def _get_log_analytics_from_diagnostic_settings():
    ...
    response = requests.get("https://management.azure.com" + _azure_ml_resource_id + "/providers/microsoft.insights/diagnosticSettings?api-version=2021-05-01-preview", headers=headers)
    response.raise_for_status()
...
def _install(log_analytics_resource_id):
    if os.geteuid() != 0:
        raise Exception("Installation must be performed by the root user. Please run again using sudo.")
    ...
    if os.path.exists(_local_config_path):
        ...
        _logger.debug(f"logAnalyticsResourceId after loading config file: {config['logAnalyticsResourceId']}")
    ...
    if log_analytics_resource_id is not None:
        ...
        _logger.debug(f"logAnalyticsResourceId after setting command-line parameter: {config['logAnalyticsResourceId']}")
    ...
    if config.get("logAnalyticsResourceId", None) is None:
        ...
        _logger.debug(f"logAnalyticsResourceId after querying Azure ML diagnostic settings: {config['logAnalyticsResourceId']}")

    # Sanitize the Log Analytics workspace ARM Resource ID
    config["logAnalyticsResourceId"] = _sanitize_log_analytics_resource_id(config["logAnalyticsResourceId"])
    ...
    _run("apt-get install -y --no-install-recommends --quiet wget apt-transport-https gnupg lsb-release")
    _run("wget -qO - https://aquasecurity.github.io/trivy-repo/deb/public.key | apt-key add -")
    _run("echo deb https://aquasecurity.github.io/trivy-repo/deb $(lsb_release -sc) main | tee -a /etc/apt/sources.list.d/trivy.list")
    _run("apt-get update")
...
def _uninstall():
    if os.geteuid() != 0:
        raise Exception("Uninstallation must be performed by the root user. Please run again using sudo.")
...
def _sanitize_log_analytics_resource_id(log_analytics_resource_id):
    if log_analytics_resource_id is None:
        raise ValueError("Log Analytics Workspace ARM Resource ID missing. Please provide it either via config file, command-line parameter, or Azure ML diagnostic settings.")

    log_analytics_resource_id = log_analytics_resource_id.strip()

    if len(log_analytics_resource_id.split("/")) != 9:
        raise ValueError("Log Analytics Workspace ARM Resource ID format should be /subscriptions/{subscription}/resourceGroups/{resource_group}/providers/Microsoft.OperationalInsights/workspaces/{workspace} instead of '" + log_analytics_resource_id + "'")
...
def _get_auth_environment_variables():
    ...
    return {pair[0]:pair[1] for pair in [line.split("=", 2) for line in out.stdout.splitlines()]}
...
def _parse_clamav_stdout(stdout):
    ...
    if findings != len(files):
        raise Exception(f"Failed to parse ClamAV stdout (findings: {findings}, files: {len(files)})")
        ...
        for result in data["Results"]:
            if result["Class"] == "os-pkgs":
                for vulnerability in result.get("Vulnerabilities", []):
                    findings_os.append({
                        "title": vulnerability.get("Title", vulnerability["PkgName"] + " " + vulnerability["VulnerabilityID"]),
                ...
                _logger.warning(f"Skipping unhandled vulnerability of class {result['Class']} and type {result['Type']} for file {result['Target']}. ")
...
def _filter_trivy_results(findings):
    return sorted(findings, key=lambda x: 0 if x["severity"] == "CRITICAL" else 1 if x["severity"] == "HIGH" else 2)[:50]
...
def _scan_vulnerabilities(telemetry):
    ...
    try:
        ...
        for env_name in (entry.name for entry in os.scandir("/anaconda/envs") if entry.is_dir()):
            _logger.info(f"Saving pip freeze of conda environment {env_name} to {_config_folder_path}/anaconda/{env_name}/requirements.txt")
            os.makedirs(f"{_config_folder_path}/anaconda/{env_name}", exist_ok = True)
            _run(f"/anaconda/envs/{env_name}/bin/python3 -m pip freeze > {_config_folder_path}/anaconda/{env_name}/requirements.txt")

        _logger.info("Running Trivy scan")
        _run(f"/usr/local/bin/trivy filesystem --format json --output {_config_folder_path}/trivy.json --security-checks vuln --severity HIGH,CRITICAL --ignore-unfixed /")

        findings_os, findings_python = _parse_trivy_results(f"{_config_folder_path}/trivy.json")

        _send_assessment(telemetry, "OsVulnerabilities", len(findings_os), {"findings": _filter_trivy_results(findings_os)} if len(findings_os) > 0 else None)
        _send_assessment(telemetry, "PythonVulnerabilities", len(findings_python), {"findings": _filter_trivy_results(findings_python)} if len(findings_python) > 0 else None)
        _send_health(telemetry, "ScanVulnerabilities", "Succeeded", {"elapsedTimeInS": time.time() - start_time})
        return True

    except subprocess.CalledProcessError as e:
        _send_health(telemetry, "ScanVulnerabilities", "Failed", {"error": str(e), "stdout": e.stdout, "stderr": e.stderr, "elapsedTimeInS": time.time() - start_time})
        return False
    except Exception as e:
        _logger.exception(f"Error: {e}")
        _send_health(telemetry, "ScanVulnerabilities", "Failed", {"error": str(e), "elapsedTimeInS": time.time() - start_time})
        return False
...
def _scan_malware(telemetry):
    ...
    try:
        ...
        database_option = "-d /var/lib/azsec-clamav" if os.path.exists("/var/lib/azsec-clamav") else ""
        command = f"clamscan {database_option} -r -i --exclude-dir=^/sys/ /bin /boot /home /lib /lib64 /opt /root /sbin /anaconda",
        ...
        if findings == 0 and out.returncode != 0:
            raise Exception(f"Failed to parse ClamAV stdout (findings: {findings}, exit code: {out.returncode})")

        _send_assessment(telemetry, "Malware", findings, details)
        _send_health(telemetry, "ScanMalware", "Succeeded", {"elapsedTimeInS": time.time() - start_time})
        return True

    except subprocess.CalledProcessError as e:
        _send_health(telemetry, "ScanMalware", "Failed", {"error": str(e), "stdout": e.stdout, "stderr": e.stderr, "elapsedTimeInS": time.time() - start_time})
        return False
    except Exception as e:
        _logger.exception(e)
        _send_health(telemetry, "ScanMalware", "Failed", {"error": str(e), "elapsedTimeInS": time.time() - start_time})
        return False


def _add_common_arguments(parser):
    parser.add_argument(
        "-la", "--log-analytics-resource-id", help="ARM Resource ID of the Log Analytics workspace to log telemetry to", dest="log_analytics_resource_id")
    parser.add_argument(
        "-ll", "--log-level", help="level of log messages to display (default: INFO)", dest='log_level',
        choices=['CRITICAL', 'ERROR', 'WARNING', 'INFO', 'DEBUG'])
    parser.add_argument(
        "-o", "--output", help="output (default: log-analytics)", dest='output', choices=['log-analytics', 'stdout'])
...
if __name__ == "__main__":
    ...
    try:
        # Command-line parser
        parser = argparse.ArgumentParser(description="Azure ML Compute Security Scanner")
        subparsers = parser.add_subparsers(dest="command")

        # Command: "install"
        parser_install = subparsers.add_parser("install", help="Install dependencies and start scheduled scans. Must be run as root (use sudo).")
        _add_common_arguments(parser_install)

        # Command: "uninstall"
        parser_uninstall = subparsers.add_parser("uninstall", help="Remove scheduled scans. Must be run as root (use sudo).")
        _add_common_arguments(parser_uninstall)

        # Command: "heartbeat"
        parser_heartbeat = subparsers.add_parser("heartbeat", help="Emit a telemetry heartbeat")
        ...
        # Command: "scan all"
        parser_scan_all = subparsers_scan.add_parser("all", help="Run all security scans")
        _add_common_arguments(parser_scan_all)

        # Command: "scan malware"
        parser_scan_malware = subparsers_scan.add_parser("malware", help="Scan for malware")
        _add_common_arguments(parser_scan_malware)

        # Command: "scan vulnerabilities"
        parser_scan_vulnerabilities = subparsers_scan.add_parser("vulnerabilities", help="Scan for OS and Python vulnerabilities")
        ...
        if args.command == "install":
            ...
            telemetry = StdOutTelemetry() if args.output == "stdout" else LogAnalyticsTelemetry(args.log_analytics_resource_id)
