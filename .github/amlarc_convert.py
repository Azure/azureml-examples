...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Convert test case to AMLARC-compatible files.')
    parser.add_argument('-i', '--input', required=True, help='Input test case file')
    parser.add_argument('-o', '--output', required=False, help='Output AMLARC-compatible file, if not provides, '
                                                               'replace file inplace')
    parser.add_argument('-c', '--compute-target', required=False, help='Compute target, default is "githubtest"',
                        default='githubtest')
    parser.add_argument('-it', '--instance-type', required=False, help='Instance type')
    parser.add_argument('-cr', '--common-runtime', required=False, default=False, action='store_true',
                        help='Enable common runtime explicitly, default is "false"')
    args = parser.parse_args()
    convert(args.input, args.compute_target, args.instance_type, args.common_runtime, args.output)
