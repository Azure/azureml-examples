...
# <upload_code>
az storage blob upload-batch -d $AZUREML_DEFAULT_CONTAINER/score -s examples/model-1/onlinescoring
# </upload_code>
...
# <upload_model>
az storage blob upload-batch -d $AZUREML_DEFAULT_CONTAINER/model -s examples/model-1/model
# <upload_model>
...
# <read_condafile>
CONDA_FILE=$(cat examples/model-1/environment/conda.yml)
# <read_condafile>
...
--header "Content-Type: application/json" \
--data-raw @examples/model-1/sample-request.json
# </score_endpoint>
