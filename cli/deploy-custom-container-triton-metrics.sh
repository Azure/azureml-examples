...
cp $PARENT_PATH/triton_metrics-cc-deployment.yml $BASE_PATH/deployment.yaml
cp $PARENT_PATH/triton_metrics-cc-endpoint.yml $BASE_PATH/endpoint.yaml
...
deploy_status=`az ml online-deployment show --name triton-metrics-cc-deployment --endpoint $ENDPOINT_NAME --query "provisioning_state" -o tsv`
echo $deploy_status
