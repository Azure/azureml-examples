...
def getParameters(fileName):
    ...
    with open(fileName) as f:
        lines = f.readlines()
    numLines = len(lines)
    parameterLines = ["instance_group {","dynamic_batching {","optimization {","parameters {"]
...
def printGridParameters(parameters):
    grid_parameters_str = "EM:" + parameters['execution_mode'] + "; ETC:" + parameters['inter_op_thread_count'] + "; ATC:" + parameters['intra_op_thread_count'] + "; GL:" + parameters['graph_level'] + "; MC:" + parameters['count']
    if len(parameters['preferred_batch_size']) == 0:
        grid_parameters_str = grid_parameters_str + "; No DB"
    else:
        grid_parameters_str = grid_parameters_str + "; DB:" + " ".join(parameters['preferred_batch_size'])
...
def extendDataframe(df,parameters_str,config_dict):
    ...
    for config_path, parameters in config_dict.items():
        ...
        df.loc[df['Model Config Path']==config_path,'grid_parameters']=printGridParameters(parameters)
        fg = df[df['Model Config Path']==config_path]['Throughput (infer/sec)'].sort_values()
        fglen = len(fg)
        if(fglen>2):
            df.loc[df['Model Config Path']==config_path,'max'] = (fg.iloc[fglen-1]+fg.iloc[fglen-2]+fg.iloc[fglen-3])/3
        else:
            if(fglen>1):
                df.loc[df['Model Config Path']==config_path,'max'] = (fg.iloc[fglen-1]+fg.iloc[fglen-2])/2
            else:
                df.loc[df['Model Config Path']==config_path,'max'] = fg.iloc[fglen-1]
    df.sort_values(['back_end','precision','Model Config Path','Concurrency'],inplace=True) 
    return df
...
def getBaselineConfigPath(df):
    config_paths = df[(df['count']=='1') & (df['preferred_batch_size']=='')]['Model Config Path'].unique()
...
def getDefaultConfigPath(group_df):
    return [x for x in group_df['Model Config Path'].unique() if x[-14:] == 'config_default'][0]

def partitionbybackend(df):
    limits_df = pd.DataFrame(columns = ['Backend_Precision', 'Limit', 1,2,4,8,16,32,64,128])
    partition_members = {}
    ...
    for key in keys:
        ...
        partition_members[back_end_precision+"_Baseline_GP"]=group_df[group_df['Model Config Path']==limits['Baseline']]['grid_parameters'].unique()[0]
        ...
        if len(members) > 1:
            ...
            partition_members[back_end_precision+"_Maximum_GP"]=group_df[group_df['Model Config Path']==limits['Maximum']]['grid_parameters'].unique()[0]
        ...
        for limit in limits.keys():
            ...
            for concurrency in Concurrency_values:
                value = group_df[(group_df['Model Config Path']==limits[limit]) & (group_df['Concurrency']==concurrency)]['Throughput (infer/sec)']
                if value.empty:
                    value = -1.0
                else:
                    value = float(value)
...
def update_axis_partition(ax, df, members,backend_precision_pair, pos,clr1 = plt.cm.Purples(0.99),clr2 = plt.cm.Purples(0.75),clr3 = plt.cm.Purples(0.6),clr4 = plt.cm.Purples(0.35)):
    plotted=0
    x = df.columns
    ...
    if backend_precision_pair + "_Maximum_GP" in members:
        ...
        ax.plot(x, y_m, label = 'OLive+Model Analyzer : ('+ backend_precision_pair + " : " + str(y_m.max()) + ') [' + maximumPars +']', color = clr1)
        y_b = df.loc[(slice(None), 'Baseline'), :].iloc[0]
        ax.plot(x, y_b, label = 'OLive : ('+ backend_precision_pair + " : " + str(y_b.max()) + ') [' + baselinePars +']', color = clr3)
    else:
        y_b = df.loc[(slice(None), 'Baseline'), :].iloc[0]
        ax.plot(x, y_b, label = 'Baseline : ('+ backend_precision_pair + " : " + str(y_b.max()) + ') [' + baselinePars +']', color = clr3)       
    ax.set_ylabel('Throughput (infer/sec)', fontsize = 'medium')
    ax.set_xlabel('Request Concurrency', fontsize = 'medium')
    ax.tick_params(axis='both', labelsize='small')
    ax.spines['right'].set_visible(False)
    ax.spines['top'].set_visible(False)
    ax.legend(bbox_to_anchor = (.9, pos),
          loc = 'upper right',
          frameon = False,
          fontsize = 'x-small')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--optimal_location_file", default="Optimal_ConfigFile_Location.txt")
    ...
    partition_df,partition_members,best_performing_config_file = partitionbybackend(extended_df)
    
    print("Best Performing Config File : ")
    print(config_files_locations[best_performing_config_file])
    WriteOptimalLocationFile(optimal_location_file,config_files_locations[best_performing_config_file])
    
    partition_df = removeEmpty(partition_df)

    fig, ax1 = plt.subplots(1, 1, figsize = (8, 5))
    ax1.set_title("OLive + Model Analyzer Profiling Throughput (infer/sec) Results", fontsize = 'x-large', fontweight = 'bold')
    backend_precision_pair = "tensorrt_FP16"
    df_b = partition_df.loc[([backend_precision_pair]), :]
    pos=0.6
    update_axis_partition(ax1, df_b, partition_members,backend_precision_pair,pos,clr1 = plt.cm.Greens(0.9),clr2 = plt.cm.Greens(0.75),clr3 = plt.cm.Greens(0.6),clr4 = plt.cm.Greens(0.15))
    backend_precision_pair = "CUDA_FP32"
    df_b = partition_df.loc[([backend_precision_pair]), :]
    update_axis_partition(ax1, df_b, partition_members,backend_precision_pair,pos,clr1 = plt.cm.Blues(0.9),clr2 = plt.cm.Blues(0.75),clr3 = plt.cm.Blues(0.6),clr4 = plt.cm.Reds(0.15))
