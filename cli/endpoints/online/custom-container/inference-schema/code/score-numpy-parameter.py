...
param_output_proba = NumpyParameterType(np.random.random(3)[np.newaxis, ...], enforce_column_type=False, enforce_shape=False)
param_output_cats = NumpyParameterType(category_list)

param_output = StandardPythonParameterType({"Probabilities" : param_output_proba,  
                                            "Predicted Categories" : param_output_cats})
