...
def init():
    ...
    model = BertForMaskedLM.from_pretrained(model_dir, config=(model_dir / "config.json"), local_files_only=True)
    tokenizer = AutoTokenizer.from_pretrained(model_dir, config=(model_dir / "tokenizer_config.json"), padding=True, return_tensors="pt")
    global pipe
...
param_masked_sentences = StandardPythonParameterType(["Hello my dog is [MASK] and I love her",
                                                      "What [MASK] is the show?"])
