...
def init(): 
    ...
    model = BertForMaskedLM.from_pretrained(bert_model_dir, config=(bert_model_dir / "config.json"), local_files_only=True)
    tokenizer = AutoTokenizer.from_pretrained(bert_model_dir, config=(bert_model_dir / "tokenizer_config.json"), padding=True, return_tensors="pt")
...
param_pandas_df = PandasParameterType(pd.DataFrame([{"sentence": "Hello my dog is [MASK] and I love her"},
...
param_input = StandardPythonParameterType({'bert_input' : param_pandas_df,
                                           'iris_input' : param_input_iris})
