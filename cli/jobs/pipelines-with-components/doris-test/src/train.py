...
# define functions
def train(train_loader, test_loader, model, criterion, optimizer, epoch, device, print_freq, rank):
    running_loss = 0.0
    for i, data in enumerate(train_loader, 0):
        ...
        if i % print_freq == 0:  # print every print_freq mini-batches
            ...
            run.log('loss', 1.0* running_loss / print_freq)
...
def evaluate(test_loader, model, device):
    ...
    run.log('accuracy', 1.0* correct / total)
...
def main(args):
    ...
    if not distributed or rank == 0:
        ...
        model_path = os.path.join(args.output_dir, "torch_model.pt")
