...
def add_features(df):
    ...
    df = df.withColumn("startLatr",(F.floor(df["startLat"] / (0.01))* 0.01).cast('double'))
    df = df.withColumn("startLonr",(F.floor(df["startLon"] / (0.01))* 0.01).cast('double'))
...
df = df.withColumnRenamed("tpepPickupDateTime","pickupDatetime").withColumnRenamed("tpepDropoffDateTime","dropoffDatetime").withColumnRenamed("rateCodeId","rateCode")
