...
def upload_data_and_create_jsonl_mltable_files(
    ...
    output_csv_file_path = os.path.join(
        dataset_dir, "mmeft_chxray_multilabel_dataset.csv"
    )
...
def read_dataframe_from_csv(csv_file_path: str, dataset_dir: str) -> pd.DataFrame:
    ...
    df[image_column_name] = df.apply(lambda x: image_to_str(x[image_column_name]), axis=1)
...
if __name__ == "__main__":
    ...
    # Read a sample row from dataset
    # Initialize dataset specific fields
    dataset_dir = os.path.join(args.data_path, "ChXray")
    csv_file_path = os.path.join(dataset_dir, "chxray_multilabel_dataset.csv")
