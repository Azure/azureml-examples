...
if hvd.rank() == 0:
    ...
    callbacks.append(tf.keras.callbacks.ModelCheckpoint(os.path.join(output_dir, 'checkpoint-{epoch}.h5')))
...
mnist_model.fit(dataset, steps_per_epoch=args.steps_per_epoch // hvd.size(), callbacks=callbacks, epochs=args.epochs, verbose=verbose)