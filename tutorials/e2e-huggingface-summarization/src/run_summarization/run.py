...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        raw_datasets = load_dataset(data_args.dataset_name, data_args.dataset_config)
    else:
        data_files = {}
        ...
        raw_datasets = load_dataset("jsonl", data_files=data_files)
    ...
    if data_args.source_prefix is None and "t5" in model.config.architectures[0].lower():
        logger.warning(
    ...
    if training_args.do_train:
        if data_args.max_samples > 0:
            max_train_samples = min(
                len(raw_datasets["train"]), data_args.max_samples
            )
    ...
    if training_args.do_eval:
        if data_args.max_samples > 0:
            max_eval_samples = min(
                len(raw_datasets["validation"]), data_args.max_samples
            )
    ...
    if training_args.do_train:
        ...
        if model_args.registered_model_name is not None:
            model = Model.register(
                ...
                tags={"type": "huggingface", "task":"summarization", "dataset":f"{data_args.dataset_name}"},
