...
print(
    "Argument 1(columns to keep): %s" % str(args.useful_columns.strip("[]").split("\;"))
)
print(
    "Argument 2(columns renaming mapping): %s"
...
new_df = (raw_data.to_pandas_dataframe().dropna(how="all").rename(columns=columns))[
    useful_columns
]
