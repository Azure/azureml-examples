...
from ml_wrappers.model.wrapped_classification_without_proba_model import WrappedClassificationWithoutProbaModel
...
class ExplanationWrapper(mlflow.pyfunc.PythonModel):
    ...
    def __init__(self, model_uri, baseline_df, target_column, task_type, categorical_features=None):
        super().__init__()
        ...
        # Wrap model
        wrapped_model = wrap_model(model, task_type, baseline_df, target_column, classes)
    ...
    def load_context(self, context):
        ...
        try:
            conda_file = mlflow.pyfunc.get_model_dependencies(context.artifacts['model'], format='conda')
    ...
    def predict(self, context, model_input):
        ...
        for index, data in model_input.iterrows():
            localExplanation = self.rai_insights.explainer.request_explanations(local=True, data=pd.DataFrame(data).T)
...
def wrap_model(model, task_type, baseline_df, target_column, classes=None):
    if task_type == CLASSIFICATION_TASK_TYPE:
        if not hasattr(model, 'predict_proba'):
            ...
            return WrappedClassificationModel(model, model.predict_proba, DatasetWrapper(baseline_df.drop(columns=[target_column])))
    else:
        return WrappedRegressionModel(model, model.predict, DatasetWrapper(baseline_df.drop(columns=[target_column])))
