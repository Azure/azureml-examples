...
parser.add_argument('--config_name', type=str, default='mnli', help='config name of the dataset')
# argument to save a fraction of the dataset
parser.add_argument('--fraction', type=float, default=0.1, help='fraction of the dataset to save')
# add an argument to specify the directory to download the dataset to
parser.add_argument('--download_dir', type=str, default='./', help='directory to download the dataset to')
...
for split in get_dataset_split_names(args.dataset, config_name=args.config_name):
    if split == args.split:
        ...
        dataset.select(range(int(dataset.num_rows * args.fraction))).to_json(os.path.join(args.download_dir, f'{split}.jsonl'))
