...
class InputDataset(ABC):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.train_data_file_name, self.test_data_file_name, self.eval_data_file_name = None, None, None
...
class QALocalInputDataset(InputDataset):
    ...
    def __init__(self, dir="", dataset_name="cqa", train_samples="512", test_samples="256", eval_samples=None):
        """Initialize the class."""
        super().__init__()
        self.dir = dir
        if train_samples is not None:
            self.train_data_file_name = dir + dataset_name + "_train_" + str(train_samples) + ".jsonl"
        if test_samples is not None:
            self.test_data_file_name = dir + dataset_name + "_test_" + str(test_samples) + ".jsonl"
        if eval_samples is not None:
            self.eval_data_file_name = dir + dataset_name + "_eval_" + str(eval_samples) + ".jsonl"
...
class QAHuggingFaceInputDataset(InputDataset):
    ...
    def load_hf_dataset(self,
        ...
        if val_split_name is not None:
            ...
            train_val_data = full_dataset[train_split_name].select(range(train_sample_size + val_sample_size))
            train_data = train_val_data.select(range(train_sample_size))
            val_data = train_val_data.select(range(train_sample_size, train_sample_size + val_sample_size))
            test_data = full_dataset[test_split_name].select(range(test_sample_size))
...
class QAPromptGenerator(PromptGenerator):
    ...
    def generate_prompt(self, qa_input):
        ...
        answer_choices = ['({}) {}'.format(labels[i], choice_list[i]) for i in range(len(labels))]
        answer_choices = '\n'.join(answer_choices)

        self.qa_user_prompt = self.qa_user_prompt_template.format(question=qa_input['question'], answer_choices=answer_choices)
...
class NLIPromptGenerator(PromptGenerator):
    ...
    def generate_prompt(self, nli_input):
        ...
        self.nli_user_prompt = self.nli_user_prompt_template.format(premise=premise, hypothesis=hypothesis)

        final_prompt = {
            "messages": [
                {
                    "role": "system",
                    "content": self.nli_system_prompt
                },
                {
                    "role": "user",
                    "content": self.nli_user_prompt
                }
            ]
        }
...
class QASyntheticDatasetBuilder(SyntheticDatasetBuilder):
    ...
    def build_dataset(self, dataset, file_name=None, write_labels_to_separate_file=False):
        """Build the dataset."""
        if self.inference_pointer is not None or self.prompt_builder is not None:
            ...
            if file_name is not None:
                if write_labels_to_separate_file:
                    self._write_to_file(data=final_dataset, fname=file_name + "_label_" + ".jsonl")
                    self._write_to_file(data=no_label_dataset, fname=file_name + ".jsonl")
                else:
...
class NLISyntheticDatasetBuilder(SyntheticDatasetBuilder):
    ...
    def build_dataset(self, dataset, file_name=None, write_labels_to_separate_file=False):
        ...
        if self.inference_pointer is not None or self.nli_prompt_builder is not None:
            ...
            if file_name is not None:
                if write_labels_to_separate_file:
                    self._write_to_file(data=no_label_dataset, fname=file_name + "_label_" + ".jsonl")
                    self._write_to_file(data=final_dataset, fname=file_name + ".jsonl")
                else:
                    self._write_to_file(data=final_dataset, fname=file_name + ".jsonl")
                print("Write to file complete")
