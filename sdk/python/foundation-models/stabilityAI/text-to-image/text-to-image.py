...
def save_key_and_generate_image(prompt, output_format, negative_prompt, seed, size, progress=gr.Progress(track_tqdm=True)):
...
def generate_image(prompt, output_format, negative_prompt, seed, size):
    ...
    headers = {
        "Authorization": f"{STABILITY_KEY}",
        "Accept": "application/json"
    }

    response = requests.post(AZURE_HOST, headers=headers, json=params)
...
demo = gr.Interface(
    ...
    inputs=[
        ...
        gr.Slider(minimum=0, maximum=10000, step=1, label="Seed (optional)", value=0),
        gr.Radio(choices=["672x1566", "832x1216", "768x1366", "836x1254", "916x1145", "1024x1024", "1145x916", "1254x836", "1366x768", "1566x672"], label="Image Size", value="1024x1024")
    ],
