...
If you create an AmlCompute cluster of one of these RDMA-capable, InfiniBand-enabled sizes, such as `Standard_ND40rs_v2`, the OS image will come with the Mellanox OFED driver required to enable InfiniBand preinstalled. To leverage this for training, you will need to set the path to this driver in the Docker image you use for your training job:

```
LD_LIBRARY_PATH=$ LD_LIBRARY_PATH:/opt/azureml/mlnx_ofed/16.04/usermode/lib
```
