...
if [[ ! -z "${RUN_BOOTSTRAP:-}" ]]; then
    ...
    for aks_compute in "${configure_aks_cluster[@]}"; do
      (
        ...
        "$SCRIPT_DIR"/sdk_helpers.sh ensure_aks_compute "${aks_compute}" 1 3 "STANDARD_D4S_V3"
      ) &
    done
    ...
    for arc_compute in "${configure_arc_cluster[@]}"; do
      (
        ...
        "$SCRIPT_DIR"/sdk_helpers.sh ensure_aks_compute "${arc_compute}" 1 3 "STANDARD_D4S_V3"
