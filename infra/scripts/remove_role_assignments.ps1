...
try
{
    ...
    staleRoleAssignments = Get-AzRoleAssignment -ResourceGroupName "$ResourceGroupName" | Where-Object {($_.ObjectType -eq $OBJTYPE) -and ($_.RoleDefinitionName -match "Storage Blob Data Reader|AzureML Metrics Writer \(preview\)|AcrPull|Contributor|AzureML Data Scientist|AzureML Registry User")}
