...
def main():
    ...
    if training_args.do_train:
        ...
        if int(rank) == 0:
            ...
            subprocess.call(["python", "-m", "onnxruntime.transformers.models.t5.convert_to_onnx", "-m", str(trained_model_path), "--output", str(trained_model_path / "T5-small"), "--use_external_data_format"])
            conda_env = {
            ...
            #    artifact_path=str(trained_model_path), conda_env=conda_env
            #)
            
            #mlflow.transformers.save_model(
            #    transformers_model=components,
            #    path=str(trained_model_path), conda_env=conda_env,
            #    pickle_module=pickle
            #)

            # using azureml evaluate package
            #model_info = azmlflow.hftransformers.log_model(
            #    model, artifact_path=str(trained_model_path),
            #    conda_env=conda_env, hf_conf=misc_conf
            #)
