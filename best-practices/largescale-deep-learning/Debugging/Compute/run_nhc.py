...
def parse_output(output_file):
    with open(output_file) as output_log:
        ...
        if full_errors:
            print("Errors found while running node health checks. Removing node from cluster.")
            subprocess.run(["fallocate", "-l", "1T", "bigfile"])
            raise Exception("Failures were found while running the node health checks. Please see the std_log_process.txt files under the 'outputs and logs' tab of the job for more information." + full_errors)
    ...
    with open(conf_file_name) as conf_file:
        ...
        while index != -1:
            ...
            if first_endline != -1 and first_space != -1:
                check_end = first_endline if first_endline < first_space else first_space
...
def print_check_desc(checks):
    ...
    for check in checks:
        ...
        if check in check_descriptions:
            print("| " + check + (" "*(max_check_name_length - len(check))) + "| " + check_descriptions[check])
...
if __name__ == "__main__":
    ...
    # get sku type
    r = requests.get('http://169.254.169.254/metadata/instance/compute/vmSize?api-version=2021-01-01&format=text', headers={'Metadata':'true'}, timeout=10)
